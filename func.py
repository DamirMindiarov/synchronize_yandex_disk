...
def get_all_file_paths(directory: str) -> list[dict]:
    ...
    for filename in os.listdir(directory):
        ...
        file_paths.append({"file_path": filepath,
                           "last_modified": os.path.getmtime(filepath),
                           "filename": filename})
    return file_paths


def detect_file_change(file_path: str, last_modified: float) -> tuple[bool, float]:
...
def synchronize_with_disk(all_files: list[dict], my_disk: SyncToDisk):
    ...
    try:
        all_filenames_on_disk = [file["name"] for file in
                                 my_disk.get_info()]
    except TypeError:
